...
public class Init {
	...
	public static void main(String[] args) {
		...
		try {
			if (!(new File(Application.DIR + "/config.properties").exists())) {
				...
				ResourceManager.extract("/br/com/cnabbradesco/resources/config.properties.example", Application.DIR);
			}
			if (!(new File(Application.DIR + "/log4j2.properties").exists())) {
				System.out.println(
						"Arquivo \"log4j2.properties\" não encontrado. Criando novo template como \"log4j2.properties.example\" no mesmo contexto."
								+ "\n");
				ResourceManager.extract("/br/com/cnabbradesco/resources/log4j2.properties.example", Application.DIR);
			}
		} catch (Exception e) {
			System.out.println("\n"
	...
	private static void help() {
		...
		System.out
				.println("\n" + "Uso: java jar cnabbradesco.jar [OPÇÃO] \"ARQUIVO\" \"Application.DIRETORIO_DESTINO\"");
		System.out.println("\n" + "Opções de comandos:");
